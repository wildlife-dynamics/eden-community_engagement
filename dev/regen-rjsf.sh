...
if [ ! -x "$REGISTRY_BIN" ]; then
    echo "Error: $REGISTRY_BIN not found or not executable. Compile the workflow once first."
    exit 1
fi
...
# --- Step 1: populate known_tasks from the registry dump (mirrors
# wt_compiler.discovery's RegistryEntry -> KnownTask conversion). ---
...
out = Path('$GENERATED_DIR') / 'ecoscope_workflows_community_engagement_workflow' / 'rjsf.json'
